...
- **Abonnement propriétaire** (`abonnements_proprietaires`) : à la demande explicite de l'utilisateur, les fiches d'un propriétaire ne restent visibles publiquement que tant qu'un abonnement est valide — 10 000 FCFA / mois (30 jours), avec remise de 5 % sur 3 mois, 8 % sur 6 mois et 10 % sur 12 mois (`src/lib/constants.ts`). Le paiement est **auto-déclaré** : aucune passerelle de paiement réelle n'est intégrée (conforme à la section 5 du cahier des charges) — le propriétaire règle par le moyen électronique de son choix (Mobile Money, virement...) puis confirme lui-même sur `/proprietaire/abonnement`, ce qui active l'abonnement immédiatement. Un renouvellement prolonge l'abonnement en cours plutôt que de le remplacer. Un bandeau de rappel s'affiche dans tout l'espace propriétaire dès 7 jours avant l'échéance (ou après expiration) ; l'administrateur voit le statut de chaque propriétaire sur `/admin/comptes` avec un bouton d'envoi manuel de rappel par SMS. **Le SMS n'est pas encore branché à un fournisseur réel** (aucun compte SMS disponible au moment du développement, choix explicite de l'utilisateur) : `src/lib/sms.ts` journalise l'envoi sans l'effectuer ; brancher un vrai fournisseur (Twilio ou autre) plus tard ne demande que quelques minutes, voir les commentaires en tête de ce fichier. Une fiche dont le propriétaire n'a plus d'abonnement valide est automatiquement masquée du public (recherche, page d'accueil, vitrine propriétaire, fiche détail), sans intervention manuelle.
